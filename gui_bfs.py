...
class BFSGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Maze Solver - Breadth-First Search (BFS)")
    ...
    def setup_ui(self):
        ...
        tk.Label(control_frame, text="Algorithm: BFS", bg="#3c3f41", fg="#4fc3f7", font=("Arial", 16, "bold")).pack(side=tk.LEFT, padx=20)

        run_btn = tk.Button(control_frame, text="▶ Run BFS", bg="#4caf50", fg="white", font=("Arial", 12, "bold"), command=self.run_algorithm)
        ...
        self.status_label = tk.Label(self.root, text="Ready. Click 'Run BFS' to start.", bg="#2b2b2b", fg="#4fc3f7", font=("Arial", 14))
    ...
    def run_algorithm(self):
        ...
        self.status_label.config(text="Running BFS...", fg="#ffeb3b")

        try:
            result = bfs(self.maze_data)
        ...
        self.animate(path, visited_order, costs_info, "BFS")
    ...
    def animate(self, path, visited_order, costs_info, algo_name):
        ...
        def show_visited(index):
            if index < len(visited_order):
                ...
                if self.maze_data[r][c] not in ['S', 'G']:
                    self.canvas.itemconfig(self.rectangles[(r, c)], fill="#81d4fa")
                    # Display g depth values
                    if (r, c) in costs_info:
                        ...
                        # Update the status label to explain
                        self.cost_label.config(text=f"Visiting ({r},{c}) | Level(Depth)={info['g']}")
        ...
        def show_path(index):
            if index < len(path):
                ...
                if self.maze_data[r][c] not in ['S', 'G']:
                    ...
                    # Show the cost value on top of the cell
                    x1, y1 = c * self.cell_size, r * self.cell_size
