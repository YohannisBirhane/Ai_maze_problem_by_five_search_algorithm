...
class ASTARGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Maze Solver - A* Search Algorithm")
    ...
    def setup_ui(self):
        ...
        tk.Label(control_frame, text="Algorithm: A*", bg="#3c3f41", fg="#00e676", font=("Arial", 16, "bold")).pack(side=tk.LEFT, padx=20)

        run_btn = tk.Button(control_frame, text="▶ Run A*", bg="#4caf50", fg="white", font=("Arial", 12, "bold"), command=self.run_algorithm)
        ...
        self.status_label = tk.Label(self.root, text="Ready. Click 'Run A*' to start.", bg="#2b2b2b", fg="#4fc3f7", font=("Arial", 14))
    ...
    def run_algorithm(self):
        ...
        self.status_label.config(text="Running A* Search...", fg="#ffeb3b")

        try:
            result = astar(self.maze_data)
        ...
        self.animate(path, visited_order, costs_info, "A*")
    ...
    def animate(self, path, visited_order, costs_info, algo_name):
        ...
        def show_visited(index):
            if index < len(visited_order):
                ...
                if self.maze_data[r][c] not in ['S', 'G']:
                    self.canvas.itemconfig(self.rectangles[(r, c)], fill="#b2dfdb")
        ...
        def show_path(index):
            if index < len(path):
                r, c = path[index]
                self.cost_label.config(text=f"Current Cost: {index}")
                if self.maze_data[r][c] not in ['S', 'G']:
