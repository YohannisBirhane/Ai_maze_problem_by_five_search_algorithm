...
class UCSGUI:
    def __init__(self, root):
        self.root = root
        self.root.title("Maze Solver - Uniform Cost Search (UCS)")
    ...
    def setup_ui(self):
        ...
        tk.Label(control_frame, text="Algorithm: UCS", bg="#3c3f41", fg="#b39ddb", font=("Arial", 16, "bold")).pack(side=tk.LEFT, padx=20)

        run_btn = tk.Button(control_frame, text="▶ Run UCS", bg="#4caf50", fg="white", font=("Arial", 12, "bold"), command=self.run_algorithm)
        ...
        self.status_label = tk.Label(self.root, text="Ready. Click 'Run UCS' to start.", bg="#2b2b2b", fg="#4fc3f7", font=("Arial", 14))
    ...
    def run_algorithm(self):
        ...
        self.status_label.config(text="Running UCS...", fg="#ffeb3b")

        try:
            result = ucs(self.maze_data)
        ...
        self.animate(path, visited_order, costs_info, "UCS")
    ...
    def animate(self, path, visited_order, costs_info, algo_name):
        ...
        def show_visited(index):
            if index < len(visited_order):
                ...
                if self.maze_data[r][c] not in ['S', 'G']:
                    self.canvas.itemconfig(self.rectangles[(r, c)], fill="#b39ddb") # Purple for UCS
                    ...
                    if (r, c) in costs_info:
                        ...
                        self.cost_label.config(text=f"Visiting ({r},{c}) | g(n)={info['g']} (Path Cost)")
